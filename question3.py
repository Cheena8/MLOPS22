...
#Hyperparameters
#Values of the defaulted parameters defined
...
#PART: Compute evaluation metrics on dev and test
    ...
    if dev_Accuracy > best_dev_Accuracy:
        return test_accuracy, dev_Accuracy, hyper_params, clf
    else:
        return best_test_Accuracy, best_dev_Accuracy, best_hyper_params, clf
...
for train_index, test_index in kfold.split(reshapedImages):
    ...
    X_train, X_dev_test, y_train, y_dev_test = train_test_split(
        reshapedImages, digits.target, test_size=dev_test_frac, random_state=RANDOM_SEED 
                                                                  )
    ...
    if CLF == 'svm':
        ...
        for index,row in df.iterrows():
            best_test_Accuracy, best_dev_Accuracy, best_hyper_params, model = SVM(X_train, y_train, X_dev, y_dev, X_test, y_test, best_test_Accuracy, best_dev_Accuracy,best_hyper_params,row['gamma'],row['c'])
            print('We are skipping hyperparameter tuning by giving the same values of hyperparamater')
            print(foldNum, best_test_Accuracy, best_hyper_params)
            break
    
    if CLF =='tree':

        for index,row in dfHyperDecision.iterrows():                                                                 
            tree_best_test_Accuracy, best_dev_Accuracy, best_hyper_params, model = Decision_Tree(X_train, y_train, X_dev, y_dev, X_test, y_test, tree_best_test_Accuracy, tree_best_dev_Accuracy,tree_best_hyper_params,row['max_depth'],row['min_samples'], row['criterion'])
...
import pickle
filename = 'results/'+CLF+'_'+str(RANDOM_SEED)+'.txt'
modelfilename = 'models/'+CLF+'_gamma='+str(best_hyper_params['gamma'])+'_C='+str(best_hyper_params['C'])+'.sav'
pickle.dump(model, open(modelfilename, 'wb'))
import joblib
lines = ['test accuracy:', str(best_test_Accuracy)]
